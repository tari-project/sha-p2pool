...
- To run a specific test, add `-- --name <Test Name>` to the command line
  ...
  cargo test --release --test cucumber -- --name "Different squads should stay on their respective chains"

  # You can also use the short option -n
  cargo test --release --test cucumber -- -n "Different squads should stay on their respective chains"
  ```
...
- To run a specific file or files add `-- --input glob` to the command line
  ...
  cargo test --release --test cucumber -- --input "tests/features/Sync*"

  # If you want to run a single file make the glob specific to the filename
  cargo test --release --test cucumber -- --input "tests/features/Sync.feature"

  # You can also use the short option -i
  cargo test --release --test cucumber -- -i "tests/features/Sync*"
  ```
