...
use minotari_app_grpc::tari_rpc::{
    Block, BlockBlobRequest, BlockGroupRequest, BlockGroupResponse, BlockHeaderResponse,
    BlockHeight, BlockTimingResponse, ConsensusConstants, Empty, FetchMatchingUtxosRequest,
    GetActiveValidatorNodesRequest, GetBlocksRequest, GetHeaderByHashRequest,
    GetMempoolTransactionsRequest, GetNewBlockBlobResult, GetNewBlockResult,
    ...
    GetTemplateRegistrationsRequest, HeightRequest, HistoricalBlock, ListConnectedPeersResponse,
    ListHeadersRequest, MempoolStatsResponse, NetworkStatusResponse, NewBlockTemplate,
    NewBlockTemplateRequest, NewBlockTemplateResponse, NodeIdentity, SearchKernelsRequest,
    SearchUtxosRequest, SoftwareUpdate, StringValue, SubmitBlockResponse, SubmitTransactionRequest,
    SubmitTransactionResponse, SyncInfoResponse, SyncProgressResponse, TipInfoResponse,
    TransactionStateRequest, TransactionStateResponse, ValueAtHeightResponse,
};
...
macro_rules! proxy_simple_result {
    ($self:ident, $call:ident, $request:ident) => {
        match $self.client.lock().await.$call($request.into_inner()).await {
            ...
            Err(error) => {
                error!(
                    "Error while calling {:?} on base node: {:?}",
                    stringify!($call),
                    error
                );
                Err(error)
    ...
    pub async fn new(base_node_address: String) -> Result<Self, Error> {
        Ok(Self {
            client: Arc::new(Mutex::new(
                util::connect_base_node(base_node_address).await?,
            )),
        })
    }
}
...
impl tari_rpc::base_node_server::BaseNode for TariBaseNodeGrpc {
    ...
    ) -> Result<Response<BlockTimingResponse>, Status> {
        proxy_simple_result!(self, get_block_timing, request)
    }
    async fn get_constants(
        &self,
        request: Request<BlockHeight>,
    ) -> Result<Response<ConsensusConstants>, Status> {
    ...
    ) -> Result<Response<SoftwareUpdate>, Status> {
    ...
    type GetNetworkDifficultyStream =
        mpsc::Receiver<Result<tari_rpc::NetworkDifficultyResponse, Status>>;
    ...
    async fn get_sync_info(
        &self,
        request: Request<Empty>,
    ) -> Result<Response<SyncInfoResponse>, Status> {
    ...
    async fn get_sync_progress(
        &self,
        request: Request<Empty>,
    ) -> Result<Response<SyncProgressResponse>, Status> {
    ...
    async fn get_tip_info(
        &self,
        request: Request<Empty>,
    ) -> Result<Response<TipInfoResponse>, Status> {
    ...
    type FetchMatchingUtxosStream =
        mpsc::Receiver<Result<tari_rpc::FetchMatchingUtxosResponse, Status>>;
    ...
    type GetMempoolTransactionsStream =
        mpsc::Receiver<Result<tari_rpc::GetMempoolTransactionsResponse, Status>>;
    ...
    type GetActiveValidatorNodesStream =
        mpsc::Receiver<Result<tari_rpc::GetActiveValidatorNodesResponse, Status>>;
    ...
    type GetTemplateRegistrationsStream =
        mpsc::Receiver<Result<tari_rpc::GetTemplateRegistrationResponse, Status>>;
    ...
    type GetSideChainUtxosStream =
        mpsc::Receiver<Result<tari_rpc::GetSideChainUtxosResponse, Status>>;
