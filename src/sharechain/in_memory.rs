...
impl ShareChain for InMemoryShareChain {
    ...
    async fn generate_new_tip_block(
        ...
        log_parents[0] = if new_height >= 10 {
            match chain_read_lock.level_at_height(new_height - 10) {
        ...
        log_parents[1] = if new_height >= 20 {
            match chain_read_lock.level_at_height(new_height - 20) {
                Some(level) => level.chain_block.clone(),
                None => FixedHash::zero(),
            }
        } else {
        ...
        log_parents[2] = if new_height >= 100 {
            match chain_read_lock.level_at_height(new_height - 100) {
                Some(level) => level.chain_block.clone(),
                None => FixedHash::zero(),
            }
        } else {
        ...
        log_parents[3] = if new_height >= 2160 {
            match chain_read_lock.level_at_height(new_height - 2160) {
                Some(level) => level.chain_block.clone(),
                None => FixedHash::zero(),
            }
        } else {
