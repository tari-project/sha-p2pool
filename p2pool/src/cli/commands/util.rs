...
use crate::{
    ...
    diagnostics::{self as diagnostics_server, server::Server as DiagnosticServer},
    server::{
        self as main_server,
...
#[allow(clippy::too_many_lines)]
pub async fn diagnostics_server(
    cli: Arc<Cli>,
    args: &StartArgs,
    shutdown_signal: ShutdownSignal,
    enable_logging: bool,
) -> anyhow::Result<DiagnosticServer> {
    if enable_logging {
        let _unused = fs::remove_file(cli.base_dir().join("configs/logs.yml"));
        // logger setup
        if let Err(e) = initialize_logging(
    ...
    let mut config_builder = diagnostics_server::Config::builder();
    if let Some(p2p_port) = args.p2p_port {
        config_builder.with_p2p_port(p2p_port);
    }
    ...
    let swarm = crate::diagnostics::p2p::setup::new_swarm(&config).await?;
    let squad = config.p2p_service.squad_override.clone().unwrap_or_else(|| {
        let squad_id =
            (*swarm.local_peer_id().to_bytes().last().unwrap_or(&0) as usize) % max(1, config.p2p_service.num_squads);
        format!("{}_{}", config.p2p_service.squad_prefix.clone(), squad_id)
